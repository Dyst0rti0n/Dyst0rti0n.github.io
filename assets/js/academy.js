document.addEventListener('DOMContentLoaded', function () {
    ...
    // Update the achievement counter display
    const updateAchievementCounter = () => {
        document.getElementById('achievement-count').textContent = `${achievementsCollected}/${totalAchievements}`;
    };

    // Call update to set initial state
    updateAchievementCounter();

    // Function to increment achievements
    const incrementAchievement = () => {
        if (achievementsCollected < totalAchievements) {
            achievementsCollected++;
            localStorage.setItem('achievementsCollected', achievementsCollected);
            updateAchievementCounter();
        }
    };
    ...
    function submitPoll() {
        ...
        if (selected) {
            ...
            incrementAchievement(); // Call incrementAchievement when poll is submitted
        } else {
